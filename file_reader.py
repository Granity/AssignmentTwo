...
class FileReader(object):  # Claye
    ...
    #
    # # Claye, Works with CSV and TXT docs ################## Long Method ############
    #  def split_file(self, file_name, switch, separator=","):
    #      try:
    #          file = open(file_name, "r")
    #      except FileNotFoundError:
    #          error = (Err.get_error_message(201))
    #          return error
    #      else:
    ...
    def open_file(self, file_name, switch, separator=","):
        try:
            ...
            self.split_file(file, switch, separator)

    def split_file(self, file, switch, separator=","):
        results = False
        for line in file:
            results = FileReader.process_file(line, separator)
        # file.close()
        if results[0]:
            valid_dict = DataProcessor.send_to_validate(FileReader.dict_root, switch, results[1])
            self.write_file(valid_dict)
    ...
    def process_file(line, separator):
        ...
        fields = line.split(separator)
        checked_id = DataProcessor.validate_key(fields[0])
        if checked_id in FileReader.dict_root:
            dup_keys += 1
            FileReader.log_duplicate_row(fields)
        else:
            keep_going = FileReader.assign_data_to_fields(checked_id, fields)
        return keep_going, dup_keys

    @staticmethod
    def assign_data_to_fields(checked_id, fields):
        try:
            FileReader.dict_root.update({checked_id: {'gender': fields[1], 'age': fields[2], 'sales': fields[3],
    ...
    def save_pickle_file(self, data_to_write):  # Claye, Graham
        ...
        u = self.get_input("Are you sure you want to save data? Y/N >>> ")
        if u.upper() == "Y":
            file_target = self.get_input("Please input the filename to save to >>> ")
